...
def add_episode(topic: str, turns: list) -> str:
    ...
    # Save metadata to episodes.json
    episodes[episode_id] = {
        "id": episode_id,
        "topic": topic,
    ...
    # Save full episode data (turns with text and tts) to episodes_data
    episode_data = {
        "id": episode_id,
        "topic": topic,
        "created_at": datetime.now().isoformat(),
    ...
    episode_file = os.path.join("episodes_data", f"{episode_id}.json")
...
def get_episode(episode_id: str) -> dict:
    ...
    # First try to load from episodes_data (full data)
    episode_file = os.path.join("episodes_data", f"{episode_id}.json")
    ...
    # Fallback to metadata only from episodes.json
    episodes = load_episodes()
